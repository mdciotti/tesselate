...
Canvas2DRenderer.prototype.loadTileset = function (tileset) {
...
Canvas2DRenderer.prototype.paintTile = function (scene, tileY, tileX, x, y) {
	var map = scene.map;
	var kx = map.tileWidth * scene.scale;
	var ky = map.tileHeight * scene.scale;
	var sourceX = map.tileWidth * tileX,
		sourceY = map.tileHeight * tileY;
	var destX = -scene.viewBox[0] + x * kx,
		destY = -scene.viewBox[1] + y * ky;

	// TODO: actually load correct tile set?
	var img = map.tilesets[0].image;
	this.context.drawImage(img, sourceX, sourceY, map.tileWidth, map.tileHeight, destX, destY, kx, ky);
};
...
Canvas2DRenderer.prototype.preDraw = function () {
	this.context.save();
};

Canvas2DRenderer.prototype.postDraw = function () {
	this.context.restore();
};
